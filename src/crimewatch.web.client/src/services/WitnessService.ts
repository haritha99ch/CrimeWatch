...
export const GetWitness = async (witnessId : WitnessId) : Promise<Witness> => {
    const response = await Api.get<Witness>(`${controller}/Get/${witnessId.value}`, await getBearerToken());
    const witness = response.data;
    return witness;
}

export const GetWitnessReports = async (witnessId : WitnessId) : Promise<Report[]> => {
    const response = await Api.get<Report[]>(`${controller}/GetReports/${witnessId.value}`, await getBearerToken());
    const reports = response.data;
    return reports;
}

export const UpdateWitness = async (witness : UpdateWitnessDto) : Promise<Witness> => {
    const response = await Api.patch<Witness>(`${controller}/Update`, witness, await getBearerToken());
...
export const DeleteWitness = async (witnessId : WitnessId) : Promise<boolean> => {
    const response = await Api.delete<boolean>(`${controller}/Delete/${witnessId.value}`, await getBearerToken());
